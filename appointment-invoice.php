...
//     error_reporting(E_ALL);
	...
	if(isset($_GET['inv'])){
		$inv = $_GET['inv'];
		$edit = query_by_id("SELECT i.itime, i.due,i.paid,i.bpaid,i.taxtype,i.tax,cou.discount,cou.discount_type,cou.max_amount,cou.min_amount,cou.ccode as coupon_name,i.coupon as coupon_id,i.dis,i.doa,i.id,i.total,i.subtotal,CONCAT(i.tax,',',i.taxtype) as tax_type,i.status,i.dis as total_discount,c.name,c.id as c_id,c.cont,c.address,c.email from `app_invoice_".$branch_id."` i"
...
<body data-gr-c-s-loaded="true" cz-shortcut-listen="true" style="width: 300px;">
    
    <!-- Company profile -->
    
    <div  style="text-align:center;font-size:40px">
        <p><img src="<?php echo systemlogo($conn); ?>" alt="<?php echo systemname($conn); ?>" style="max-width:200px"/></p>
		<?php 
			$sql1="SELECT * FROM `system` where branch_id='".$branch_id."'";
			$result1=query_by_id($sql1,[],$conn);
			foreach($result1 as $row1) { ?>
			<p style="font-size:12px;max-width:250px;text-align:center;margin:0px auto;text-transform:uppercase;margin-bottom:2px;"><b><?= $row1['address']; ?></b></p>
			<p style="font-size:12px;max-width:250px;text-align:center;margin:0px auto;"><b>Contact : <?= $row1['phone']; ?></b></p>
			<?php if($row1['email']!=''){ ?>
				<p style="font-size:12px;max-width:250px;text-align:center;margin:0px auto;"><b>Email : <?= $row1['email']; ?></b></p>
			<?php } ?>
			<?php if($row1['website']!=''){ ?>
				<p style="font-size:12px;max-width:250px;text-align:center;margin:0px auto;"><b>Website : <?= $row1['website']; ?></b></p>
			<?php } ?>
			<?php if($row1['gst']!=''){ ?>
				<p style="font-size:12px;max-width:250px;text-align:center;margin:0px auto;"><b>GST No : <?= $row1['gst']; ?></b></p>
			<?php } ?>
		<?php } ?>
	</div>
	<h4 style="text-align:center;"><strong>APPOINTMENT INV.</strong></h4>
	...
	<div>
		<table style="width:100%;font-size:12px;font-weight:600;border-collapse:collapse;">
			...
			<tbody>
			    <?php
    			$sql2="SELECT ii.id as ii_iddd,imsp.aii_staffid,imsp.service_Provider,GROUP_CONCAT(b.name,' ') as name,b.id b_id,ii.client,ii.service,ii.quantity,ii.disc_row,ii.staffid,ii.price,ii.price as actual_price,s.price as service_price,s.name as service_name,s.cat,CONCAT('sr,',s.id) as service_id,s.duration as service_durr,scat.cat as cat_name from `app_invoice_items_".$branch_id."` ii"
						." LEFT JOIN `service` s on s.id=SUBSTRING_INDEX(ii.service,',',-1)"
						." LEFT JOIN `servicecat` scat on scat.id=s.cat"
						." LEFT JOIN `app_multi_service_provider` imsp on imsp.aii_staffid=ii.id"
						." LEFT JOIN `beauticians` b on b.id=imsp.service_Provider "
						." where ii.iid='$inv' and ii.branch_id='".$branch_id."' and imsp.branch_id='".$branch_id."' and ii.active=0 GROUP BY aii_staffid";
    				...
    				<tr style="font-size:12px;">
    					<td style="padding:5px 7px;vertical-align:top;"><?=getEnquiryfor($row1['service']); ?> <?php
    					$ispackage_ser = query_by_id("SELECT count(*) as total,p.name as pck_name FROM package_service_history psh LEFT JOIN `app_invoice_items_".$branch_id."` ii ON psh.invoice_id = ii.iid LEFT JOIN packages p ON psh.package_id = p.id WHERE ii.price = 0 AND psh.status='1' AND ii.id = '".$row1['ii_iddd']."' AND ii.iid ='". $edit['id']."'",[],$conn)[0];
    					...
    					<td style="padding:5px 7px;vertical-align:top;"><?= $row1['service_price'] ?></td>
    					<td style="padding:5px 7px;vertical-align:top;"><?=(EXPLODE(",",$row1['disc_row'])[0] == 'pr')?EXPLODE(",",$row1['disc_row'])[1].' %':EXPLODE(",",$row1['disc_row'])[1] ?></td>
    					<td style="padding:5px 7px;vertical-align:top;"><?= $row1['quantity']; ?></td>
    					<td style="padding:5px 7px;vertical-align:top;"><?= $row1['price'] ?></td>
    				</tr>
			<?php $count++; $sp_id++; $total_qty += $row1['quantity']; $given_discount += ($row1['service_price']-$row1['price']); } ?>
		    </tbody>
		</table>
		<hr style="margin-top:9px;margin-bottom:9px;border-color:#8c8c8c;">
		<table style="width:100%;font-size:12px;font-weight:600;border-collapse:collapse;">
		    ...
		    <tr>
		        <td rowspan="12" colspan="2" style="vertical-align:top;padding:0px 7px;">Payment Mode : <br />
		            <?php $paystatus = '';
                    	$paymethod = "SELECT pm.name as name, mpm.transaction_id FROM multiple_payment_method mpm LEFT JOIN payment_method pm ON mpm.payment_method = pm.id WHERE invoice_id='app,".$edit['id']."' and mpm.branch_id='".$branch_id."'";
		        ...
		        <td style="text-align:right;padding:0px 7px;"><?php
		            ...
		            if($coupon_dis != ''){
    					...
    					if($sub > $edit['max_amount'] && $edit['max_amount'] != 0){
    					    $sub = $edit['max_amount'];
    					}
		            }
		    ...
		    <tr>
		        ...
		        <td style="text-align:right;padding:0px 7px;"><?= number_format($given_discount,2) ?></td>
		    </tr>
		    <tr>
			...
			<tr>
				<td style="text-align:right;padding:0px 7px;"><strong>Advance Paid : </strong></td>
				<td style="text-align:right;padding:0px 7px;"><?= number_format($edit['paid'],2); ?></td>
			</tr>
			<tr>
				<td style="text-align:right;padding:0px 7px;"><strong>Due : </strong></td>
		...
		<!--</p>-->
		<!--<div>-->
		<!--    <h5 style="padding-left:10px;">T&C</h5>-->
		<!--    <ol style="font-size:12px;font-weight:600;text-align:justify;padding-left:24px;">-->
		<!--        <li>Package booked is the amount of services intiated by the Cleint however service shall be provided by the company only to the extend of amount received against services.</li>-->
		<!--        <li>Money once diposited is not refundable however it can be adjusted fowards can other pkg after deduction of 20% of amount paid</li>-->
		<!--    </ol>-->
		<!--</div>-->
	</div>
