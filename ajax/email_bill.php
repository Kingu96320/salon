...
if(isset($_POST['action']) && $_POST['action'] == 'invoice_email'){
	$inv = htmlspecialchars(trim($_POST['inv']));
	$email_id = htmlspecialchars(trim($_POST['email_address']));
	...
	$html = '';
	$html .= '<!DOCTYPE html><html lang="en" class="gr__s_bootsnipp_com"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
	...
	<body data-gr-c-s-loaded="true" cz-shortcut-listen="true" style="margin-top:30px; margin-bottom:30px;">
	    ...
	    <!-- Company profile -->
	    <h4 style="text-align:left;font-weight:600;">INVOICE <span style="font-size:14px;float:right;">Branch : '.ucfirst(branch_by_id($branch_id)).'</span></h4>
	    <div  style="font-size:40px">
	        <div style="float:left;width:200px;"><img src="'.BASE_URL.systemlogo($conn).'" alt="'.systemname($conn).'" style="max-width:120px"/></div>
			...
			if($row1['website']!=''){
				$html .= '<p style="font-size:12px;max-width:250px;margin:0px auto;"><b>Website : <a href="'.$row1['website'].'">'.$row1['website'].'</a></b></p>';
			}
			if($row1['gst']!=''){
	...
	$html .= '
	</div></div><div class="clearfix" style="float: none;width: 100%;display: inline-block;"></div>
    			...
    			$sql2="SELECT ii.id as ii_iddd,imsp.ii_id,imsp.service_Provider,GROUP_CONCAT(b.name,' ') as name,b.id b_id,ii.client,ii.service,ii.quantity,ii.disc_row,ii.staffid,ii.price,ii.actual_price,s.price as service_price,s.name as service_name,s.cat,CONCAT('sr,',s.id) as service_id,s.duration as service_durr,scat.cat as cat_name from `invoice_items_".$branch_id."` ii"
    				...
    				foreach($result2 as $row1) {
	    				...
	    				$html .= '<tr style="font-size:12px;">
	    					<td style="padding:5px 7px;vertical-align:top;">'.getEnquiryfor($row1['service']).' ';
							...
							if($ispackage_ser['total'] != 0){
	    						$html .= "(PKG. DED-".$ispackage_ser['pck_name'].")";	
	    					}    			
	    				$html .= ' </td>
...
</div><div style="text-align: center;margin-top: 20px;">Powered by <a href="https://easysalon.in/">easysalon.in</a></div>
</body></html>';
	$client_id = query_by_id("SELECT client FROM invoice_".$branch_id." WHERE id='".$inv."' AND active='0'",[],$conn)[0]['client'];
	if($client_id > 0){
		if($email_id != ''){
			query("UPDATE client SET email='".$email_id."' WHERE id='".$client_id."' AND active='0' AND branch_id='".$branch_id."'",[],$conn);
		}
		$client_email = query_by_id("SELECT email, name FROM client WHERE id='".$client_id."' AND active='0' AND branch_id='".$branch_id."'",[],$conn)[0];
		if($client_email['email'] != ''){
			$mail = new PHPMailer(true);
            $mail->From = $result1[0]['email'];
            $mail->FromName = systemname();
            $mail->addAddress($client_email['email'], $client_email['name']);
            $mail->isHTML(true);
            ...
            try {
                if($mail->send()){
					...
					$result['message'] = "Email sent on ".$client_email['email'];
					echo json_encode($result); die();
                } else {
                	$result['status'] = 3;
	               	$result['message'] = 'email_not_sent';
	               	echo json_encode($result); die();
                }
            } catch (Exception $e) {
               $result['status'] = 3;
